<template>
  <div v-if="pendingFiles.length" class="chat-box__previews">
    <div v-for="(item, index) in pendingFiles" :key="index" class="chat-preview-item">
      <img v-if="item.preview" :src="item.preview" class="chat-preview-img" :alt="item.file.name" />
      <span v-else class="chat-preview-name">{{ item.file.name }}</span>
      <button class="chat-preview-rm" aria-label="Убрать" @click="emit('removePending', index)">×</button>
    </div>
  </div>

  <p v-if="error" class="chat-box__error">{{ error }}</p>

  <div class="chat-box__foot">
    <button
      class="chat-box__attach chat-iconbtn"
    ...
    <input
      ...
      type="file"
      multiple
      accept="image/*,audio/*,.pdf,.doc,.docx,.webm,.mp4,.ogg,.m4a,.mp3"
    ...
    />
    <button
      class="chat-box__send btn btn--primary"
...
import { ref } from 'vue'
import type { PendingChatFile } from '@/composables/useChatComposer'
