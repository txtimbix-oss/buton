    ...
    <div class="pf">
      ...
      <section class="pf-banner">
        ...
        <div class="pf-info">
          <div class="pf-name">
            {{ fullName || 'Профиль' }}
            <span v-if="tierName" class="pf-vip">{{ tierName }}</span>
          </div>
          <div v-if="sinceText" class="pf-since">{{ sinceText }}</div>
          <label class="pf-photo">
            <input type="file" accept="image/*" hidden @change="onAvatarFile" />
            <span>{{ avatarUploading ? 'Загрузка…' : 'Изменить фото' }}</span>
          </label>
        </div>
        <div class="pf-bonus">
          <div class="pf-bonus-v">{{ fmt(bonusBalance) }}</div>
          <div class="pf-bonus-l">бонусов на счету</div>
          <div class="pf-bonus-c">{{ tierName }} · кэшбэк {{ tierCashback }}%</div>
        </div>
      </section>

      <!-- LEVEL PROGRESS -->
      <section v-if="!isMaxLevel && nextLevel" class="pf-level">
        <div class="pf-level-t">
          <span><b>{{ tierName }}</b> → <span class="clay">{{ nextTierName }}</span></span>
          <span>{{ fmt(totalSpent) }} / {{ fmt(nextThreshold) }} ₽</span>
        </div>
        <div class="pf-track"><div class="pf-fill" :style="{ width: progressPct + '%' }" /></div>
        <div class="pf-level-h">
          До уровня «{{ nextTierName }}» и кэшбэка {{ nextLevel.cashback }}% осталось заказов на
          <b>{{ fmt(remainingToNext) }} ₽</b>.
        </div>
      </section>
        ...
        <section class="pf-card">
          ...
          <form @submit.prevent="save">
            <div class="pf-fld">
              ...
              <input class="pf-inp" v-model="form.firstName" required />
            </div>
            <div class="pf-fld">
              <label>Фамилия <span class="req">*</span></label>
              <input class="pf-inp" v-model="form.lastName" required />
            </div>
            <div class="pf-fld">
              <label>Телефон</label>
              <input class="pf-inp" v-model="form.phone" inputmode="tel" placeholder="+7 999 000-00-00" />
            </div>
            <div class="pf-fld">
              <label>Email</label>
              <input class="pf-inp" :value="form.email" disabled />
              <div class="pf-hint">Email изменить нельзя — обратитесь в поддержку</div>
            </div>
            <div v-if="success" class="pf-ok">{{ success }}</div>
            <div v-if="error" class="pf-err">{{ error }}</div>
            <div class="pf-btns">
              <button class="pf-save" :disabled="saving">{{ saving ? 'Сохраняем…' : 'Сохранить изменения' }}</button>
              <button type="button" class="pf-ghost" @click="reset">Отмена</button>
            </div>
          </form>
        </section>
        ...
        <div class="pf-col">
          ...
          <section class="pf-card">
            ...
            <div class="pf-toggle-row">
              ...
              <button type="button" class="pf-toggle" :class="{ on: notif.orderStatus }" aria-label="Статус заказов" @click="toggleNotification('orderStatus')" />
            </div>
            <div class="pf-toggle-row">
              <div class="pf-tt"><div class="n">Новости и акции</div><div class="d">Скидки и сезонные предложения</div></div>
              <button type="button" class="pf-toggle" :class="{ on: notif.news }" aria-label="Новости и акции" @click="toggleNotification('news')" />
            </div>
          </section>
          ...
          <section class="pf-ref">
            ...
            <div class="pf-ref-code">
              ...
              <button type="button" :class="{ copied: refCopied }" @click="copyRef">
                {{ refCopied ? 'Ссылка скопирована' : 'Копировать ссылку' }}
              </button>
            </div>
      ...
      <div class="pf-medals">
        <div v-for="m in medals" :key="m.key" class="pf-medal" :class="m.cls">
          <div class="pf-mc">
            <span class="pf-mc-ic">{{ m.icon }}</span>
            <span v-if="m.done" class="pf-mok">✓</span>
          </div>
...
import { achievementsApi, type AchievementsResponse } from '@/api/achievements'
...
const bonusBalance = computed(() => user.value?.bonusBalance ?? 0)
...
const sinceText = computed(() => {
  ...
  if (!d) return ''
  try {
    return `В клубе с ${new Date(d).toLocaleDateString('ru-RU', { month: 'long', year: 'numeric' })}`
  } catch {
    return ''
  }
})
...
async function copyRef() {
  const link = `${shopUrl}/?ref=${referralCode.value ?? ''}`
...
const ACH_EMOJI: Record<string, string> = {
  first_order: '🛍️', regular: '🌸', generous: '💎', critic: '⭐', legend: '👑',
}
const DEFAULT_ACH = [
...
const achData = ref<AchievementsResponse | null>(null)
onMounted(async () => {
  try { achData.value = await achievementsApi.get() } catch { /* остаётся дефолт */ }
})
const medals = computed(() => {
  const items = (achData.value?.items?.length
    ? achData.value.items
    : DEFAULT_ACH.map(a => ({
        ...a, earned: earnedAchievements.value.includes(a.key),
        progressPct: 0, current: 0, target: 0, status: '',
      }))) as Array<{ key: string; name: string; description: string; earned: boolean; progressPct?: number; current?: number; target?: number }>
  return items.map((a) => {
    const done = !!a.earned
    ...
    return {
      ...
      cls: done ? 'done' : inProgress ? 'current' : 'lock',
      pct: a.progressPct ?? 0,
      stat: done ? 'Открыто' : inProgress ? `В процессе · ${a.current ?? 0}/${a.target ?? 0}` : 'Закрыто',
    }
  })
})
const achEarned = computed(() => achData.value?.summary?.earned ?? medals.value.filter(m => m.done).length)
const achTotal = computed(() => achData.value?.summary?.total ?? medals.value.length)
</script>

<style scoped>
/* токены старого дизайна, которых нет в кабинете — локально */
.pf {
...
.page-title, .page-sub { display: none; } /* старый заголовок не нужен — есть баннер */
...
.pf-ava {
  ...
  background: var(--pf-green-wash); color: var(--green);
...
.pf-vip {
  ...
  background: var(--clay); color: #fff; padding: 4px 11px; border-radius: 20px;
}
.pf-since { font-size: 13px; color: oklch(0.82 0.015 150); margin-top: 5px; }
.pf-photo { display: inline-block; margin-top: 12px; cursor: pointer; }
.pf-photo span {
  display: inline-flex; align-items: center; height: 38px; padding: 0 16px;
  border: 1px solid oklch(1 0 0 / .3); border-radius: 10px;
  font-size: 13.5px; font-weight: 600; color: #fff; background: oklch(1 0 0 / .08);
  transition: background .15s;
}
.pf-photo:hover span { background: oklch(1 0 0 / .16); }
...
.pf-bonus-c {
  ...
  color: var(--clay); background: oklch(0.62 0.11 47 / .2); padding: 4px 11px; border-radius: 20px;
}
...
@media (max-width: 860px) { .pf-two { grid-template-columns: 1fr; } }
...
.pf-inp {
  width: 100%; height: 46px; border: 1px solid var(--pf-line-strong); border-radius: 10px;
  padding: 0 13px; font-size: 14.5px; font-family: inherit; color: var(--ink); background: var(--card);
}
...
.pf-err { font-size: 13px; color: var(--error, #c0392b); margin-top: 4px; }
.pf-btns { display: flex; gap: 10px; margin-top: 20px; flex-wrap: wrap; }
.pf-save {
  height: 46px; padding: 0 20px; border: none; border-radius: 10px; cursor: pointer;
  background: var(--green); color: #fff; font-family: inherit; font-weight: 600; font-size: 14.5px;
  transition: background .15s, opacity .15s;
}
.pf-save:hover { background: var(--green-l); }
.pf-save:disabled { opacity: .6; cursor: default; }
.pf-ghost {
  height: 46px; padding: 0 18px; border: 1px solid var(--pf-line-strong); border-radius: 10px; cursor: pointer;
  background: var(--card); color: var(--ink); font-family: inherit; font-weight: 600; font-size: 14.5px;
...
.pf-ref-code .code {
  ...
  background: oklch(1 0 0 / .12); border: 1px dashed oklch(1 0 0 / .4); border-radius: 9px; padding: 11px 16px;
}
.pf-ref-code button {
  padding: 0 16px; border-radius: 9px; cursor: pointer;
  border: 1px solid oklch(1 0 0 / .3); color: #fff; background: oklch(1 0 0 / .08);
  font-family: inherit; font-weight: 600; font-size: 13.5px; transition: background .15s, border-color .15s;
}
...
.pf-medal { background: var(--card); border: 1px solid var(--line); border-radius: var(--r-md); box-shadow: var(--shadow); padding: 22px 14px 18px; text-align: center; }
